...
class VehicledataDao {
  ...
  //Adicionado para funcionaliadade de novos vehicleData
  add(vehicledata) {
    return new Promise((resolve, reject) => {
        ...
        `,
            ...
            function (err) {
                if (err) {
                    console.log(err);
                    return reject('Can`t register new vehicle data');
                }
                console.log(`Vin ${vehicledata.vin} registered!`)
                resolve();
            });
    });
  }
  ...
  findByVin(vin) {
    return new Promise((resolve, reject) => this._db.get(
        `SELECT * FROM vehicledata WHERE vehicledata_vin = ?`,
        [vin],
        (err, row) => {
            if (err) {
                console.log(err);
                return reject('Can`t find vehicledata');
            }
